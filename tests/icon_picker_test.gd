...
func _case_1_present(main: Node) -> bool:
	...
	var picker := main.get_node_or_null("Title/IconPicker")
	if title == null:
		return _fail("case 1: Title missing")
	if picker == null:
		return _fail("case 1: Title/IconPicker missing")
	if not title.visible:
		return _fail("case 1: Title should be visible")
	if not picker.visible or not picker.is_visible_in_tree():
		return _fail("case 1: IconPicker should be visible while the title is shown")
...
func _case_5_layout(main: Node) -> bool:
	...
	var picker := main.get_node_or_null("Title/IconPicker") as Control
	var theme_picker := main.get_node_or_null("Title/ThemePicker") as Control
	var top_five := main.get_node_or_null("Title/TopFiveLabel") as Control
	if picker == null or theme_picker == null or top_five == null:
		return _fail("case 5: missing Title children")
	var picker_rect: Rect2 = picker.get_global_rect()
	var theme_rect: Rect2 = theme_picker.get_global_rect()
	var top_rect: Rect2 = top_five.get_global_rect()
	if picker_rect.intersects(theme_rect):
		return _fail("case 5: IconPicker intersects ThemePicker %s vs %s" % [picker_rect, theme_rect])
	if picker_rect.intersects(top_rect):
		return _fail("case 5: IconPicker intersects TopFiveLabel %s vs %s" % [picker_rect, top_rect])
